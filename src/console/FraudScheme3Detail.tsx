...
// 差异化：双分数（自研欺诈汇总分 0-100 + 智察分同盾原生）、5 大欺诈维度、4 张多源欺诈单项卡（六核心收敛为四宫格）、
//        8 步欺诈运算链路、移除证件照与一切信息核验重复内容、右侧导航 4 条目；操作栏直接复用信息核验 VerifyOps 交互逻辑。
import { useState } from 'react'
import { useNavigate, useSearchParams } from 'react-router-dom'
import { Badge, Button, DetailHeader, Panel } from '../components/ui'
import {
  buildFraudScheme3Report,
  type Conclusion,
  type RiskLevel,
  type SingleResult,
  type RulePromptType,
  ...
  type ScoreComponent,
  type CrossCheck,
} from './fraudScheme3Report'
import { useModule } from '../store'
import { FraudScheme3ActionBar, type FraudScheme3Row, type FraudScheme3WorkStatus, type FraudScheme3SysResult } from './FraudScheme3Ops'
...
export default function FraudScheme3Detail() {
  const nav = useNavigate()
  const [params] = useSearchParams()
  const sysParam = (params.get('sys') as FraudScheme3SysResult) ?? '处理中'
  const workParam = (params.get('work') as FraudScheme3WorkStatus) ?? '待确认'
  ...
  const d = buildFraudScheme3Report(variantId)

  const [fraudRow, setFraudScheme3Row] = useState<FraudScheme3Row>(() => ({
    id: sampleId ?? d.appId,
    name: d.name,
    product: '—',
    channel: '—',
  ...
  const applyFraud = (next: Partial<FraudScheme3Row>) => setFraudScheme3Row((r) => ({ ...r, ...next }))
  ...
  return (
    <div className="space-y-6">
      <DetailHeader
        ...
        backLabel="返回欺诈识别"
        onBack={() => nav('/console/cr/fraud-s3')}
      />
      ...
      <div className="lg:flex lg:gap-6">
        <div className="min-w-0 flex-1 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-card">
            <FraudScheme3ActionBar row={fraudRow} onApply={applyFraud} flash={flash} showView={false} />
...
function ConflictCard({
  ...
  const srcLabel: Record<string, string> = {
    blacklist: '黑名单', device: '设备', behavior: '行为', network: '网络/图谱', graph: '图谱',
  }
  const riskSource = c.items[0]?.source ? (srcLabel[c.items[0].source] ?? c.items[0].source) : '—'
  return (
    <div
      id={`conflict-${c.id}`}
      className={cn(
      ...
      {!collapsed && (
        <>
          <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
            <div className="rounded-md bg-white/70 px-2.5 py-1.5 text-xs">
              <span className="font-medium text-slate-400">风险来源：</span>
              <span className="font-semibold text-slate-700">{riskSource}</span>
            </div>
            <div className="rounded-md bg-white/70 px-2.5 py-1.5 text-xs sm:col-span-2">
              <span className="font-medium text-slate-400">风险原文：</span>
          ...
          <div className="mt-2 text-xs text-slate-500">
            <span className="font-medium text-slate-400">系统判定逻辑：</span>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-slate-200/60 pt-3">
            <Button variant="ghost" size="sm" onClick={onExempt}>豁免风险</Button>
            <Button variant="ghost" size="sm" onClick={onNote}>录入核实备注</Button>
            <Button variant="ghost" size="sm">上传凭证</Button>
          </div>
        </>
      )}
    </div>
