...
export function CollectionOverview() {
  ...
  const active = cases.filter((c) => ['催收中', '承诺还款', '待分案'].includes(c.status)).length;
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1360 }}>
      <PageShell title="催收总览" crumb="零售信贷风控 / 催收管理 / 催收总览" subtitle="逾期案件分阶段催收的实时总览 · 数据来源本地样例 JSON"
...
export function CollectionCases() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1360 }}>
      <PageShell title="催收案件" crumb="零售信贷风控 / 催收管理 / 催收案件" subtitle="逾期案件队列 · 按阶段/状态/催收员筛选，行点击查看详情并记录触达"
        actions={<><Sam label="催收样例" value="collectionData.json" /><Cal label="实时统计" /></>} />
...
export function CollectionStrategy() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1360 }}>
      <PageShell title="催收策略" crumb="零售信贷风控 / 催收管理 / 催收策略" subtitle="按逾期阶段配置催收策略（M0 提醒 → M3+ 委外）· 数据保存到本地样例 JSON"
...
export function CollectionRecords() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1360 }}>
      <PageShell title="催收记录" crumb="零售信贷风控 / 催收管理 / 催收记录" subtitle="全量催收触达记录 · 数据来自本地样例 JSON"
        actions={<><Sam label="催收样例" value="collectionData.json" /><Cal label="实时统计" /></>} />
