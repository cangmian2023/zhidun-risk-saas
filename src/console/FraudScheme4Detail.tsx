...
export default function FraudScheme4Detail() {
  ...
  // 异常值（欺诈分）越大风险越高，阈值配色由模板 grades 驱动（见 ScoreVisual）
      ...
      <div className="lg:flex lg:gap-6">
        <div className="min-w-0 flex-1 space-y-4">
          ...
          <Panel id="score">
            ...
            <div className="mt-4">
              <ScoreVisual sd={s4Tpl?.scoreDisplay} rawScore={d.fraudScore} />
            </div>
          ...
          <Panel title={basicSec?.name || '用户基本信息'} id={basicSec?.id || 'basic_info'} actions={<DisplayModeToggle reportType="fraud" sectionId={basicSec?.id || 'basic_info'} />}>
            {basicMode === 'list' ? (
              ...
              <div className={fieldGridClass(basicMode)}>
                {d.basic.map((f) => (
                  <div key={f.key} className="flex items-center justify-between gap-2 rounded-lg border border-slate-100 px-3.5 py-2.5">
                    <span className="text-sm text-slate-400">{f.label}</span>
                    <span className="text-sm font-medium text-ink-900">{f.value}</span>
                  </div>
                ))}
