...
export type FinalDecision = 'reject' | 'pass'
...
// ---------------- 终审操作记录表 ----------------
function FinalRecordTable({ rows }: { rows: FinalRecord[] }) {
  return (
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50 text-left text-xs text-slate-500">
            ...
            <th className="px-3 py-2 font-medium">原因 / 备注 & 附件</th>
            ...
            rows.map((r) => (
              <tr
                ...
                className={cn(
                  ...
                  r.highlight ? 'bg-rose-50/70' : 'odd:bg-white even:bg-slate-50/40',
                )}
              >
                <td className="px-3 py-2">
                  <Badge kind={r.badge}>{r.action}</Badge>
                </td>
                ...
                <td className="px-3 py-2">
                  {r.reviewStatus ? <Badge kind="amber">{r.reviewStatus}</Badge> : <span className="text-slate-300">—</span>}
                </td>
              </tr>
            ))
          )}
...
export function FinalOpsCard({
  ...
  const [recs, setRecs] = useState<FinalRecord[]>(initialRecords)
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-3">
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-500">系统结论</span>
          <Badge kind={decision === 'reject' ? 'red' : 'green'}>{sysText}</Badge>
        </div>
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <Button variant="ghost" onClick={() => setModal('globalNote')}>
            全局风险备注
          </Button>
          {/* 互斥切换按钮 */}
          {decision === 'reject' ? (
            <button
              type="button"
              onClick={() => setModal('forceReview')}
              className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-rose-600 px-3.5 py-2 text-sm font-medium text-white transition hover:bg-rose-700"
            >
              强制人工复审
            </button>
          ) : (
      ...
      {/* 终审操作记录 */}
      <div>
        <div className="mb-2 text-xs font-medium text-slate-500">终审操作记录</div>
        <FinalRecordTable rows={recs} />
      </div>
