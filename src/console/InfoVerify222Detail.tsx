...
import { computeSectionScore, matchGrade, scoreForVerifySys, computeReportTotal, getAuditFlowByGrade, type SectionConfig, type ReviewResult } from './reportTemplateData'
import sample from './infoVerify222Sample.json'
import detailSamples from './infoVerify222DetailData.json'
import listRows from './infoVerify222Data.json'
...
export default function InfoVerify222Detail() {
  ...
  const row = urlId ? (listRows as any[]).find((r) => r.id === urlId) : undefined
  ...
  const sampleData: any = (segGrade && (detailSamples as any)[segGrade]) || sample
  ...
  const bizButtons = (tpl.businessFlow ?? []).filter(bf => bf.gradeId === grade?.grade).flatMap(bf => bf.flowGraphs ?? [])
  ...
  const manualsOf = (fg: any) => (fg?.nodes ?? []).filter((n: any) => n.results && n.results.length)
  ...
  // 3.3 流程步进：activeFlow=当前操作的流程下标；flowStep=该流程内已完成的人工节点数
  ...
  // 审批弹窗：当前激活流程的「下一个待审批节点」（flowStep 对应的人工人工节点）
  const [auditIdx, setAuditIdx] = useState<number | null>(null)
  const curFlow = auditIdx != null ? bizButtons[auditIdx] : undefined
  const curStep = Math.min(flowStep, manualsOf(curFlow).length - 1)
  const auditFlow = auditIdx != null && grade && curFlow ? getAuditFlowByGrade(tpl, grade.grade, auditIdx, Math.max(0, curStep)) : null
  const applyAudit = (p: { result: ReviewResult; checks: string[]; opinionText: string; fileName: string }) => {
    if (!curFlow) return
    const manuals = manualsOf(curFlow)
    const node = manuals[Math.max(0, curStep)]
    const af = getAuditFlowByGrade(tpl, grade?.grade ?? '', auditIdx ?? 0, Math.max(0, curStep))
    ...
    setAuditTrail((t) => [...t, { node: node?.buttonName ?? node?.label ?? '审批', operator: op }])
    // 3.5 操作日志：自动追加
    setRuntimeLogs((l) => [...l, {
      time: new Date().toLocaleString('zh-CN', { hour12: false }),
      actor: op,
      action: '人工审核',
      detail: `${node?.buttonName ?? node?.label ?? '审批'}：${p.result}${p.opinionText ? `｜意见：${p.opinionText}` : ''}`,
    }])
    setFlowStep((s) => s + 1)
    setAuditIdx(null)
  }
  ...
  // 按钮渲染（3.3/3.4）：未激活=入口按钮；激活中=当前待审批节点按钮；流程完成=按 end.showButton 决定是否显示「已办结」
  // 初始已完成判定：用户数据 workStatus 有值且非枚举[0]（如 通过/拒绝/驳回 = 流程已走完）
  const initialDone = !!((row as any)?.workStatus) && (row as any)?.workStatus !== statusEnum[0]
  const flowButton = (fi: number): { label: string; done: boolean; hidden: boolean } => {
    ...
    const manuals = manualsOf(fg)
    if (activeFlow !== fi) {
      if (initialDone) return { label: '已办结', done: true, hidden: end?.showButton === false }
      return { label: start?.buttonName ?? fg?.name ?? '操作', done: false, hidden: false }
    }
    if (flowStep < manuals.length) {
      const node = flowStep === 0 ? start : manuals[flowStep]
      return { label: flowStep === 0 ? (start?.buttonName ?? fg?.name ?? '操作') : (node?.buttonName ?? node?.label ?? '操作'), done: false, hidden: false }
    }
  ...
  return (
    <div className="space-y-6 min-h-screen bg-slate-50">
      <DetailHeader title={`贷前审核 · 信息核验222 · ${computedAt}`} subtitle={`进件号 ${reportId}${row ? ` · 申请人 ${row.name}` : ''}`} backLabel="返回列表" onBack={() => nav('/console/cr/pre-verify-222')} />
      ...
      <div className="lg:flex lg:gap-6">
        <div className="min-w-0 flex-1 space-y-4 p-5 lg:pl-5 lg:pr-0">
          ...
          {contentSecs.map((s, idx) => {
            ...
            const raw = (sampleData as any)[s.id]
          ...
          {/* 操作日志 */}
          {tpl.showOpLog && <Panel id="oplogs" title="操作日志"><Tpl f="showOpLog" v={tpl.showOpLog} /><div className="space-y-2">{[...((sampleData as any).op_logs ?? []), ...runtimeLogs].map((l: any, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg border border-slate-100 px-3 py-2 text-sm"><span className="mt-0.5 shrink-0 text-xs text-slate-400">{V(l.time)}</span><span className="mt-0.5 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-600">{V(l.actor)}</span><span className="mt-0.5 font-medium text-ink-900">{V(l.action)}</span><span className="mt-0.5 text-slate-500">{V(l.detail)}</span>{i >= ((sampleData as any).op_logs ?? []).length && <Dat f="本次操作" v="实时记录" />}</div>)}</div></Panel>}
        </div>
