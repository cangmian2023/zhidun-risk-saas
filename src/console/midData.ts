...
export interface MidCustomer {
  ...
  modelScoreHistory?: ModelScoreHistoryPoint[]; // 模型能力/历史：智察/智信/智融 逐月轨迹
}

/* 三模型分历史轨迹点（智察/智信/智融 逐月轨迹） */
...
export function withCustGraph(c: MidCustomer): MidCustomer {
  ...
  const scores = c.scores ?? (() => {
    const mk = (score: number, range: [number, number], unit: string, hint: string): ModelScoreItem => ({ score, range, unit, hint });
