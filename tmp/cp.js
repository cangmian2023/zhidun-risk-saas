...
function KV({ k, v, danger }) {
  return /* @__PURE__ */ jsxs("div", { style: { display: "flex", justifyContent: "space-between", gap: 10, fontSize: 12, padding: "2px 0" }, children: [
    /* @__PURE__ */ jsx("span", { style: { color: "#94A3B8", whiteSpace: "nowrap" }, children: k }),
...
function RelationGraph({ graph, theme, onPick, onTheme, sel, nodeMap }) {
  const W = 760;
  const H = 470;
  const cx = W / 2;
  const cy = H / 2;
  ...
  const pos = useMemo(() => {
    ...
    nodes.forEach((node, i) => {
      if (node.type === "self") {
        ps[node.id] = { x: cx, y: cy };
        return;
      }
      const ang = Math.PI * 2 * i / Math.max(n, 1);
      ps[node.id] = { x: cx + Math.cos(ang) * (k * 1.6), y: cy + Math.sin(ang) * (k * 1.6) };
    });
    let temp = W * 0.12;
      ...
      for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
          ...
          if (d < minDist) {
            ...
            const ux2 = dx / (d || 1), uy2 = dy / (d || 1);
            disp[nodes[i].id].x += ux2 * push;
            disp[nodes[i].id].y += uy2 * push;
            disp[nodes[j].id].x -= ux2 * push;
            disp[nodes[j].id].y -= uy2 * push;
          }
          const fr = k * k / d2;
          const ux = dx / d, uy = dy / d;
          disp[nodes[i].id].x += ux * fr;
          disp[nodes[i].id].y += uy * fr;
          disp[nodes[j].id].x -= ux * fr;
          disp[nodes[j].id].y -= uy * fr;
        }
      }
      active.forEach((e) => {
        ...
        const fa = d * d / k;
        const ux = dx / d, uy = dy / d;
        disp[e.source].x += ux * fa;
        disp[e.source].y += uy * fa;
        disp[e.target].x -= ux * fa;
        disp[e.target].y -= uy * fa;
      });
      nodes.forEach((nd) => {
        const p = ps[nd.id];
        disp[nd.id].x += (cx - p.x) * 0.03;
        disp[nd.id].y += (cy - p.y) * 0.03;
      });
      const cool = temp * (1 - it / 500);
      nodes.forEach((nd) => {
        const p = ps[nd.id];
        p.x += Math.max(-cool, Math.min(cool, disp[nd.id].x));
        p.y += Math.max(-cool, Math.min(cool, disp[nd.id].y));
  ...
  const highRisk = nodes.filter((n) => n.risk === "\u9AD8\u5371").length;
  return /* @__PURE__ */ jsxs("div", { style: { position: "relative" }, children: [
    /* @__PURE__ */ jsxs("div", { style: { display: "flex", flexWrap: "wrap", gap: 14, alignItems: "center", fontSize: 12, color: "#64748B", marginBottom: 8 }, children: [
      ...
      highRisk > 0 && /* @__PURE__ */ jsxs("span", { style: { color: "#DC2626" }, children: [
        "\u9AD8\u5371\u8282\u70B9 ",
        highRisk
      ] })
    ] }),
    /* @__PURE__ */ jsx("div", { style: { position: "absolute", top: 36, right: 10, zIndex: 20, display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end", maxWidth: "70%" }, children: (graph.themes ?? ["\u7EFC\u5408"]).map((th) => /* @__PURE__ */ jsx(
      "button",
      {
        onClick: () => onPick(null),
        style: {
          ...
          borderRadius: 999,
          border: "1px solid",
          borderColor: th === theme ? THEME_COLOR[th] ?? "#8B5CF6" : "#E2E8F0",
          background: th === theme ? THEME_COLOR[th] ? `${THEME_COLOR[th]}14` : "#F5F3FF" : "#fff",
          color: th === theme ? THEME_COLOR[th] ?? "#6D28D9" : "#475569",
          cursor: "pointer"
        },
        onClick: () => {
          onTheme(th);
    ...
    /* @__PURE__ */ jsxs(
      ...
      {
        viewBox: `0 0 ${W} ${H}`,
        style: { width: "100%", height: "auto", background: "#F8FAFC", borderRadius: 12, border: "1px solid #E2E8F0", display: "block" },
        onClick: () => onPick(null),
        children: [
          active.map((e, i) => {
            ...
            return /* @__PURE__ */ jsxs("g", { children: [
              /* @__PURE__ */ jsx("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: e.danger ? "#DC2626" : THEME_COLOR[e.theme] ?? "#CBD5E1", strokeWidth: e.danger ? 1.8 : 1.1, strokeDasharray: e.danger ? "4 2" : void 0, strokeOpacity: 0.85 }),
              /* @__PURE__ */ jsx("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: "transparent", strokeWidth: 12, style: { cursor: "pointer" }, onClick: (ev) => {
                ev.stopPropagation();
                onPick({ kind: "edge", edge: e });
              } }),
              e.danger && /* @__PURE__ */ jsx("text", { x: mx, y: my - 3, textAnchor: "middle", fontSize: 9, fontWeight: 600, fill: "#DC2626", style: { paintOrder: "stroke", stroke: "#fff", strokeWidth: 3, strokeLinejoin: "round" }, children: e.rel })
            ] }, i);
          }),
          nodes.map((n) => {
            const p = pos[n.id];
            ...
            return /* @__PURE__ */ jsxs("g", { style: { cursor: "pointer" }, onClick: (ev) => {
              ev.stopPropagation();
              onPick({ kind: "node", node: n });
            }, children: [
              /* @__PURE__ */ jsx("circle", { cx: p.x, cy: p.y, r, fill: c, fillOpacity: isHi ? 0.2 : n.risk === "\u5173\u6CE8" ? 0.14 : 0.1, stroke: isHi ? "#DC2626" : c, strokeWidth: isHi ? 2.2 : n.risk === "\u5173\u6CE8" ? 1.8 : 1.3 }),
              /* @__PURE__ */ jsx("text", { x: p.x, y: p.y - 1, textAnchor: "middle", fontSize: n.type === "self" ? 12 : n.type === "company" ? 11 : 10, fontWeight: 600, fill: c, children: n.name.length > 6 ? n.name.slice(0, 5) + "\u2026" : n.name }),
              /* @__PURE__ */ jsx("text", { x: p.x, y: p.y + 12, textAnchor: "middle", fontSize: 9, fill: "#64748B", children: n.rel }),
              !!n.openAlerts && /* @__PURE__ */ jsxs("g", { children: [
                /* @__PURE__ */ jsx("circle", { cx: p.x + r - 2, cy: p.y - r + 2, r: 9, fill: "#DC2626", stroke: "#fff", strokeWidth: 1.5 }),
                /* @__PURE__ */ jsx("text", { x: p.x + r - 2, y: p.y - r + 5.5, textAnchor: "middle", fontSize: 10, fontWeight: 700, fill: "#fff", children: n.openAlerts })
              ] })
            ] }, n.id);
          })
        ]
    ...
    /* @__PURE__ */ jsx(GraphSelectionCard, { sel, nodeMap, onClose: () => onPick(null) })
  ] });
}
function GraphSelectionCard({ sel, nodeMap, onClose }) {
  if (!sel) {
    return /* @__PURE__ */ jsxs("div", { style: { position: "absolute", left: 10, top: 36, width: 220, background: "rgba(255,255,255,.92)", border: "1px solid #E2E8F0", borderRadius: 10, padding: "8px 12px", fontSize: 12, color: "#94A3B8" }, children: [
      "\u70B9\u51FB\u56FE\u4E0A ",
  ...
  if (sel.kind === "node") {
    const n = sel.node;
    return /* @__PURE__ */ jsxs("div", { style: { position: "absolute", left: 10, top: 36, width: 230, background: "rgba(255,255,255,.97)", border: "1px solid #E2E8F0", borderRadius: 10, boxShadow: "0 8px 24px rgba(15,23,42,.14)", padding: "10px 12px", zIndex: 30 }, children: [
      /* @__PURE__ */ jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }, children: [
        /* @__PURE__ */ jsx("span", { style: { fontSize: 13, fontWeight: 700, color: "#0F172A" }, children: n.name }),
        /* @__PURE__ */ jsx("button", { onClick: onClose, style: { border: "none", background: "none", cursor: "pointer", color: "#94A3B8", fontSize: 15, lineHeight: 1 }, children: "\xD7" })
      ] }),
  ...
  return /* @__PURE__ */ jsxs("div", { style: { position: "absolute", left: 10, top: 36, width: 230, background: "rgba(255,255,255,.97)", border: "1px solid #E2E8F0", borderRadius: 10, boxShadow: "0 8px 24px rgba(15,23,42,.14)", padding: "10px 12px", zIndex: 30 }, children: [
    /* @__PURE__ */ jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }, children: [
      /* @__PURE__ */ jsx("span", { style: { fontSize: 13, fontWeight: 700, color: "#0F172A" }, children: "\u5173\u7CFB\u5C5E\u6027" }),
      /* @__PURE__ */ jsx("button", { onClick: onClose, style: { border: "none", background: "none", cursor: "pointer", color: "#94A3B8", fontSize: 15, lineHeight: 1 }, children: "\xD7" })
    ] }),
...
function CustProfile() {
  ...
  return /* @__PURE__ */ jsxs("div", { style: { padding: 24, maxWidth: 1360 }, children: [
    ...
    tab === "\u5173\u7CFB\u7F51\u7EDC" && /* @__PURE__ */ jsxs(Fragment, { children: [
      /* @__PURE__ */ jsxs(Panel, { title: `\u5173\u7CFB\u56FE\u8C31 \xB7 ${relTheme}`, desc: /* @__PURE__ */ jsxs("span", { children: [
        "\u878D\u5408\u8054\u7CFB\u4EBA\u3001\u5171\u503A\u3001\u8D44\u91D1\u3001\u62C5\u4FDD\u3001\u8BBE\u5907\u7B49\u591A\u7EF4\u5173\u7CFB \xB7 \u70B9\u51FB\u8282\u70B9/\u5173\u7CFB\u770B\u5C5E\u6027 \xB7 ",
        /* @__PURE__ */ jsx(Sam, { value: "custProfileData.ts" })
      ] }), children: [
        /* @__PURE__ */ jsx(RelationGraph, { graph: cur.relationGraph, theme: relTheme, onPick: setRelSel, onTheme: setRelTheme, sel: relSel, nodeMap: relNodeMap }),
        /* @__PURE__ */ jsxs("div", { style: { display: "flex", gap: 14, fontSize: 11, color: "#64748B", marginTop: 10, flexWrap: "wrap" }, children: [
          Object.keys(TYPE_COLOR).map((k) => /* @__PURE__ */ jsxs("span", { style: { display: "inline-flex", alignItems: "center", gap: 4 }, children: [
          ...
          /* @__PURE__ */ jsxs("span", { style: { display: "inline-flex", alignItems: "center", gap: 4 }, children: [
            /* @__PURE__ */ jsx("span", { style: { width: 16, height: 16, borderRadius: "50%", border: "2px dashed #DC2626", display: "inline-block" } }),
            "\u9AD8\u5371/\u98CE\u9669\u8FB9"
          ] })
        ] })
      ] }),
      /* @__PURE__ */ jsx(Panel, { title: "\u5173\u7CFB\u4EBA\u6E05\u5355\uFF08\u878D\u5408\u8054\u7CFB\u4EBA / \u56FE\u8C31\u8282\u70B9\uFF09", desc: /* @__PURE__ */ jsxs("span", { children: [
        "\u53BB\u91CD\u5408\u5E76\u7684\u5173\u7CFB\u4EBA \xB7 \u70B9\u51FB\u67E5\u770B\u5C5E\u6027 \xB7 ",
        /* @__PURE__ */ jsx(Sam, { value: "custProfileData.ts" })
      ] }), className: "mt-3", children: /* @__PURE__ */ jsx("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8 }, children: cur.relationGraph.nodes.filter((n) => n.type !== "self" && (relTheme === "\u7EFC\u5408" || cur.relationGraph.edges.some((e) => e.theme === relTheme && (e.source === n.id || e.target === n.id)))).map((n) => /* @__PURE__ */ jsxs(
        "div",
        {
          onClick: () => setRelSel({ kind: "node", node: n }),
          style: { border: "1px solid #E2E8F0", borderRadius: 10, padding: "8px 12px", background: n.risk === "\u9AD8\u5371" ? "#FEF2F2" : "#fff", cursor: "pointer" },
          children: [
            /* @__PURE__ */ jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [
              /* @__PURE__ */ jsx("span", { style: { fontSize: 13, fontWeight: 600, color: "#334155" }, children: n.name }),
              /* @__PURE__ */ jsx(Badge, { kind: n.risk === "\u9AD8\u5371" ? "red" : n.risk === "\u5173\u6CE8" ? "amber" : "blue", children: n.rel })
            ] }),
            /* @__PURE__ */ jsxs("div", { style: { fontSize: 11, color: "#94A3B8", marginTop: 4, lineHeight: 1.5 }, children: [
              n.phone && /* @__PURE__ */ jsxs("span", { children: [
                "\u7535\u8BDD\uFF1A",
                n.phone,
                "\u3000"
              ] }),
